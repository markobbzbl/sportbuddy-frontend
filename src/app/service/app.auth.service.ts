...
export class AppAuthService {
  ...
  private _accessToken = window.sessionStorage.getItem('access_token');
...
public getRoles(): Observable<string[]> {
  ...
  const resourceRoles = this._decodedAccessToken?.resource_access?.sportbuddy?.roles ?? [];

  const allRoles = [...realmRoles, ...resourceRoles];

  const resultArr = allRoles.map((r: string) => r.replace(/^role_/i, ''));
